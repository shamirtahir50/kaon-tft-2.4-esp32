////////////////////////////////////////////////////////////////////
//            2.4 inch TFT LCD ST7789 240 x 320 esp32             //
////////////////////////////////////////////////////////////////////
...
boolean startsample = false;      // when startsampling option is pressed on Screen2 it turns true
boolean restarting = true;
boolean startsampling = false;   // turn it true to display screen 14
boolean startsamplingupdate = false;   // turn it true to update screen 14
...
String DisplayDate = "16, Oct", DisplayYear = "23";
String DisplayTime = "15:15";
...
String DisplayErrorMsg, Displaydelaytime;         // 00:00:00
String FlowDisplay;
//int delaytime = 100;
...
void setup()
{
  ...
  timedelayhour = constrain(timedelayhour, 0, 48);
  timedelayminutes = constrain(timedelayminutes, 0, 59);
  tempunitflag = readruntimehour(tempunitflag, 8);
  tempunitflag = constrain(tempunitflag, 0, 2);
  flowunitflag = readruntimehour(flowunitflag, 9);
  flowunitflag = constrain(flowunitflag, 0, 2);
  bpunitflag = readruntimehour(bpunitflag, 10);
  bpunitflag = constrain(bpunitflag, 0, 2);
  updates = readruntimehour(updates, 11);
  timedelayminutes = constrain(timedelayminutes, 0, 5);
  //      if(tempunitflag == 0){
  //        TemperatureUnits = "C";
  //      }
  //      if(tempunitflag == 1){
  ...
  attachInterrupt(digitalPinToInterrupt(0), ISR_1, CHANGE);
  attachInterrupt(digitalPinToInterrupt(0), ISR_2, CHANGE);
  attachInterrupt(digitalPinToInterrupt(0), ISR_3, CHANGE);
...
void loop()
{
  ...
  if (Screen1 == true) {
    ...
    Screen2 = true;
    background_display();
    timendate(DisplayTime, DisplayDate, ""); //Time and Date Func. TBC //Done from my side
    battary(5);
    headerunderline();
    footer();
    if (flowmode == 0) {
  ...
  if (Screenenter == true && Screen3 == true && startyesnotransition == 0) {
    ...
    Screen14_1 = true;
    startsample = true;
    Screen_15DelayScreen(displaydelaytime);       // Pass Delay time here //TBC
    Serial.println("Screen_3Reset");
  }
  if(Screen14_1 == true){
    Screen_15DelayUpdate(displaydelaytime);       // update delay time after every second 
  }
  if(startsamplingupdate == true){              // update Screen 14 values after every second
    startsamplingupdate = false;                                
    Screen14_1 = false;
    DisplayScreen_14_Update(DisplayMotion, DisplayTemp, DisplayHumidity, Displayflow, DisplayBackPressure, TemperatureUnits, BackPressureUnits, Flowrateunits, DisplayVolume, DisplayTimeHours, flow, FlowDisplay, flowmode);
    }
  if(startsampling == true){               // Screen 14 values displayed once delaytime ends
    startsampling = false;
    Screen14_1 = false;
  ...
  if(restarting == true){                  // when restarting is true
    restarting = false;
    Screen_15Restart();
   }
  ...
  if (Screenenter == true && Screen6_1 == true  && PERMISSIONCALIBRATE_no == true) {
    Screenenter = false;
    Screen6_1 = false;
    startcalibrate = false;                                  // calibration started  
    stopcalibrate = true;                                  // calibration stopped
    PERMISSIONCALIBRATE_no = false;
    PERMISSIONCALIBRATE_yes = false;
    flowyesnotransition = 0;
    Screen2 = true;
  ...
  if (Screenenter == true && Screen10_1 == true && confirmyesnotransition == 1) {
    ...
    // Adjust RTC Time and Date function Here TBC
  }
  //    // Screen 11
...
//    Serial.println("Factory Reset");
  ...
  //  // Screen 17
  /////////////////////////////////////////////////////////////////////////////
  if (Screenenter == true && settings_transition == 6 && Screen8 == true) {
    Screenenter = false;
    Screen8 = false;
    Screen17 = true;
    Screen_17();
    ...
    if (Screenenter == true && Screen17 == true) {
    Screenenter = false;
    Screen8 = true;
    Screen17 = false;
    Screen_17();
